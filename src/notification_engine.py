...
def enqueue_or_send(conn, channels: list[NotificationChannel], job_id: int, job: Job, summary: str) -> None:
    ...
    if mode == "immediate":
        _notify_all(channels, "send_job_match", job, summary)
    else:
...
def maybe_flush(conn, channels: list[NotificationChannel], config: Config) -> None:
    ...
    entries = [(_job_from_row(row), row["queue_id"]) for row in pending]
    ...
    if mode == "rate_limited":
        ...
        for job, _ in to_release:
            _notify_all(channels, "send_job_match", job, _summary_for(job, config))
        db.mark_notifications_sent(conn, [qid for _, qid in to_release])
        ...
        if len(entries) >= batch_count:
            for job, _ in entries:
                _notify_all(channels, "send_job_match", job, _summary_for(job, config))
            db.mark_notifications_sent(conn, [qid for _, qid in entries])
        ...
        if due:
            digest_entries = [(job, _summary_for(job, config)) for job, _ in entries]
            for channel in channels:
